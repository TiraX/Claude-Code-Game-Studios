...
Then use `ask the user` to capture feedback:
...
Use `ask the user` to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` via the available multi-agent tool using gate TD-SYSTEM-BOUNDARY (`.claude/docs/director-gates.md`) before proceeding to priority assignment.**
...
Use `ask the user` to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` via the available multi-agent tool using gate PR-SCOPE (`.claude/docs/director-gates.md`) before writing the index.**
...
**After the systems index is written, spawn `creative-director` via the available multi-agent tool using gate CD-SYSTEMS (`.claude/docs/director-gates.md`).**
...
Use `ask the user` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `/design-system` completes, use `ask the user`:
...
After the systems index is created (or after designing some systems), present next actions using `ask the user`:
