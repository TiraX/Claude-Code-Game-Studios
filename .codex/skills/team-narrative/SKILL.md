...
> Usage: `/team-narrative [narrative content description]` — describe the story content, scene, or narrative area to work on (e.g., `boss encounter cutscene`, `faction intro dialogue`, `tutorial narrative`). Do not use `ask the user` here; output the guidance directly.
...
Delegate in parallel — issue all three multi-agent calls simultaneously before waiting for any result:
...
sub-agents spawned via the available multi-agent tool. Each sub-agent enforces the "May I write to [path]?"
