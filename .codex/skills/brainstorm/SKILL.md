...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `ask the user`** at key decision points throughout brainstorming:
   ...
   `ask the user` to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an ask the user with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `ask the user` for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab `ask the user` with these exact tab labels:
...
Present all three. Then use `ask the user` to capture the selection.
...
Ask these as `ask the user` calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `ask the user`:
...
If the user selects B, C, or D, make the revision, then use `ask the user` again:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` via the available multi-agent tool in parallel before moving to Phase 5. Issue both multi-agent calls simultaneously — do not wait for one before starting the other.**
...
Collect both verdicts, then present them together using a two-tab `ask the user`:
...
- **Target platform**: Use `ask the user` — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `ask the user` — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, spawn `technical-director` via the available multi-agent tool using gate TD-FEASIBILITY (`.claude/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via the available multi-agent tool using gate PR-SCOPE (`.claude/docs/director-gates.md`).**
...
5. Use `ask the user` for write approval:
...
If [B]: ask which section to revise using `ask the user` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `ask the user` — "Ready to write the updated concept document?"
