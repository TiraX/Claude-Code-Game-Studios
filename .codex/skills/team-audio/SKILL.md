...
> Usage: `/team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`). Do not use `ask the user` here; output the guidance directly.
...
to sub-agents spawned via the available multi-agent tool. Each sub-agent enforces the "May I write to [path]?"
