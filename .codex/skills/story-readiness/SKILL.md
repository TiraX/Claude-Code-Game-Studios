...
**Scope:** `$ARGUMENTS[0]` (blank = ask user via ask the user)
...
If no argument is given, use `ask the user`:
...
Spawn `qa-lead` via the available multi-agent tool using gate **QL-STORY-READY** (`.claude/docs/director-gates.md`).
...
- **GAPS [list]** → surface the specific gaps to the user via `ask the user`:
