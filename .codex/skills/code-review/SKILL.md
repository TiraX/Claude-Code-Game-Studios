...
Spawn all applicable specialists simultaneously via the available multi-agent tool — do not wait for one before starting the next.
...
For Logic and Integration stories, also spawn `qa-tester` via the available multi-agent tool in parallel with the engine specialists. Pass:
