...
**If no argument is provided**, do not fail — ask instead. Use `ask the user`:
...
2. **Questions**: Ask what is needed to draft this section. Use `ask the user`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `ask the user` to capture the decision.
...
- Use `ask the user`: "Want an ASCII wireframe as part of this spec?"
...
Use `ask the user` to surface any open questions on accessibility tier:
...
Use `ask the user` to step through items in groups of 3-4, not all at once.
