...
After completing the engine audit above, spawn the **primary engine specialist** via the available multi-agent tool for a domain-expert second opinion:
...
Use `ask the user` for the write approval:
...
3. **Offer options** via ask the user with three choices:
