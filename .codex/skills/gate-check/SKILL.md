...
Before generating the final verdict, spawn all four directors as **parallel subagents** via the available multi-agent tool using the parallel gate protocol from `.claude/docs/director-gates.md`. Issue all four multi-agent calls simultaneously — do not wait for one before starting the next.
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using `ask the user`.
...
- **No accessibility requirements doc?** → Use `ask the user` to offer to create it now:
  ...
  - If "Create it now": use a second `ask the user` to ask for the tier:
