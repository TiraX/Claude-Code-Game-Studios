...
Spawn `technical-director` via the available multi-agent tool using gate **TD-MANIFEST** (`.claude/docs/director-gates.md`).
...
- **CONCERNS** → surface via `ask the user` with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
