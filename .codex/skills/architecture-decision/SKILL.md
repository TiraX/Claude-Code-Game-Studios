...
a **confirm/adjust** prompt using `ask the user` — not open-ended questions.
...
decisions remain, present each one as a separate `ask the user` with the proposed
...
4.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** via the available multi-agent tool to validate the drafted ADR:
...
4.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` via the available multi-agent tool using gate **TD-ADR** (`.claude/docs/director-gates.md`):
...
5. **Write approval** — Use `ask the user`:
...
After the ADR is written (and registry optionally updated), close with `ask the user`.
