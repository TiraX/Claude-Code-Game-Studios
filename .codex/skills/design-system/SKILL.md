...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `ask the user`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `ask the user` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `ask the user` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `ask the user`. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use `ask the user` with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use `ask the user`:
...
spawn `creative-director` via the available multi-agent tool:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents via the available multi-agent tool in parallel:
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents via the available multi-agent tool in parallel:
...
For required systems: **spawn `art-director` via the available multi-agent tool** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
Before finalizing the GDD, spawn `creative-director` via the available multi-agent tool using gate **CD-GDD-ALIGN** (`.claude/docs/director-gates.md`).
...
**When delegating via the available multi-agent tool tool**:
...
- The main session presents the agent's output to the user via `ask the user`
- The user decides; the main session writes to file
...
2. **ask the user** at every decision point (Explain -> Capture pattern):
