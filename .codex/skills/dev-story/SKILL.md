...
If they differ, use `ask the user` before proceeding:
...
specialist to spawn via the available multi-agent tool.
...
Spawn the chosen programmer agent(s) via the available multi-agent tool with the full context package:
...
- **File writes are delegated** — all source code, test files, and evidence docs are written by sub-agents spawned via the available multi-agent tool. Each sub-agent enforces the "May I write to [path]?" protocol individually. This orchestrator does not write files directly.
