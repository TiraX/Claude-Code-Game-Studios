...
- If it exists: read it, find the first context (system/level/character) with any asset at status "Needed" but no spec file written yet, and use `ask the user`:
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use `ask the user`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
Present the full identified list to the user. Use `ask the user`:
...
Spawn specialist agents based on review mode. **Issue all multi-agent calls simultaneously — do not wait for one before starting the next.**

### Full mode — spawn in parallel:

**`art-director`** via the available multi-agent tool:
...
**`technical-artist`** via the available multi-agent tool:
...
After presenting all specs, use `ask the user`:
