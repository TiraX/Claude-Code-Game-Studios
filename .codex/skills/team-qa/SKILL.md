...
Spawn `qa-lead` via the available multi-agent tool to review all in-scope stories and produce a QA strategy.
...
Present the qa-lead's full strategy to the user, then use `ask the user`:
...
Spawn `qa-tester` via the available multi-agent tool for each story (run in parallel where possible), providing:
- The story file path
...
Use `ask the user` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `ask the user` for each:
...
After each FAIL result: use `ask the user` to collect the failure description, then spawn `qa-tester` via the available multi-agent tool to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-lead` via the available multi-agent tool to produce the sign-off report using all results from Phases 4–6.
...
If any spawned agent (via the available multi-agent tool) returns BLOCKED, errors, or cannot complete:

1. **Surface immediately**: Report "[AgentName]: BLOCKED — [reason]" to the user before continuing to dependent phases
2. **Assess dependencies**: Check whether the blocked agent's output is required by subsequent phases. If yes, do not proceed past that dependency point without user input.
3. **Offer options** via ask the user with choices:
