...
Use `ask the user` to confirm proceeding or reduce scope.
...
Spawn `release-manager` via the available multi-agent tool. Ask them to produce a rollback plan covering:
...
1. Spawn `lead-programmer` via the available multi-agent tool with:
...
3. Spawn `qa-tester` via the available multi-agent tool to verify: does the bug reproduce after the fix?
...
Spawn `qa-lead` via the available multi-agent tool with:
