...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` via the available multi-agent tool using gate **QL-STORY-READY** (`.claude/docs/director-gates.md`).
...
Use `ask the user` to close with context-aware next steps:
